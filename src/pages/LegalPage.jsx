...
const sections = {
  privacidade: {
    ...
    intro: 'Este documento descreve como o ArcMode trata dados no funcionamento atual do projeto.',
    items: [
      ...
      ['Contato', <>Para dúvidas, solicitações de suporte ou pedidos relacionados a dados pessoais, escreva para {supportLink}. Não envie senhas, tokens ou outros dados sensíveis.</>]
    ]
  },
  termos: {
    title: 'Termos de Uso',
    ...
    items: [
      ['Finalidade do projeto', 'O ArcMode é uma aplicação de produtividade e planejamento para organizar tarefas, projetos, compromissos e tempo. O planejamento automático usa regras de organização; não é aconselhamento profissional nem uma promessa de resultado.'],
      ...
      ['Contato', <>Para dúvidas ou solicitações sobre o projeto, escreva para {supportLink}.</>]
    ]
  }
};
...
export default function LegalPage({ type }) {
  ...
  return <main className="min-h-screen bg-slate-50 text-slate-900"><header className="border-b border-slate-200 bg-white"><div className="max-w-4xl mx-auto px-5 py-5 flex items-center justify-between"><Link to="/" className="flex items-center gap-2 font-bold"><Logo size={36}/>ArcMode</Link><Link to="/" className="text-sm font-semibold text-blue-700 hover:text-blue-900">Voltar ao início</Link></div></header><article className="max-w-4xl mx-auto px-5 py-12 sm:py-16"><p className="text-sm font-semibold text-amber-700">Projeto pessoal em desenvolvimento</p><h1 className="mt-2 text-3xl sm:text-4xl font-black tracking-tight">{content.title}</h1><p className="mt-4 text-slate-600 leading-relaxed">{content.intro}</p><p className="mt-2 text-sm text-slate-500">Última atualização: 11 de setembro de 2026.</p><div className="mt-10 space-y-8">{content.items.map(([title,text])=><section key={title}><h2 className="text-xl font-bold">{title}</h2><p className="mt-2 text-slate-600 leading-relaxed">{text}</p></section>)}</div></article></main>;
}
