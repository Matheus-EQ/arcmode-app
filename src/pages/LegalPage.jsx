...
const sections = {
  privacidade: {
    ...
    intro: 'Este documento descreve o funcionamento atual do NeuroSync e precisa de revisão antes da divulgação pública.',
    items: [
      ...
      ['Contato', '[PREENCHER ANTES DA PUBLICAÇÃO: endereço de e-mail ou formulário de contato do responsável pelo projeto].']
    ]
  },
  termos: {
    title: 'Termos de Uso',
    intro: 'Estes termos refletem um projeto pessoal em desenvolvimento e precisam de revisão antes da divulgação pública.',
    items: [
...
export default function LegalPage({ type }) {
  ...
  return <main className="min-h-screen bg-slate-50 text-slate-900"><header className="border-b border-slate-200 bg-white"><div className="max-w-4xl mx-auto px-5 py-5 flex items-center justify-between"><Link to="/" className="flex items-center gap-2 font-bold"><Logo size={36} professional/>NeuroSync</Link><Link to="/" className="text-sm font-semibold text-blue-700 hover:text-blue-900">Voltar ao início</Link></div></header><article className="max-w-4xl mx-auto px-5 py-12 sm:py-16"><p className="text-sm font-semibold text-amber-700">Rascunho para revisão</p><h1 className="mt-2 text-3xl sm:text-4xl font-black tracking-tight">{content.title}</h1><p className="mt-4 text-slate-600 leading-relaxed">{content.intro}</p><p className="mt-2 text-sm text-slate-500">Última atualização: 9 de setembro de 2026.</p><div className="mt-10 space-y-8">{content.items.map(([title,text])=><section key={title}><h2 className="text-xl font-bold">{title}</h2><p className="mt-2 text-slate-600 leading-relaxed">{text}</p></section>)}</div></article></main>;
}
